...
{
   [ExecuteInEditMode]
   public class AutoCamV : PivotBasedCameraRig
   {
      [SerializeField]
      private float m_MoveSpeed = 5; // How fast the rig will move to keep up with target's position
      [SerializeField]
      private float m_TurnSpeed = 4; // How fast the rig will turn to keep up with target's rotation
      [SerializeField]
      ...
      [SerializeField]
      private float m_SpinTurnLimit = 60;// The threshold beyond which the camera stops following the target's rotation. (used in situations where a car spins out, for example)
      [SerializeField]
      ...
      //----------------georgi---------------------
      [SerializeField]
      private float RotateCamSpeed = 80;
      float secondsReturnCam = 90;
      float returnCam = 0;
      //-------------------------------------
      ...
      {
         //-----------------georgi-------------------------
         // Rotate Camera around the car with mouse and pause return
         if (Input.GetAxis("Mouse X") < 0) { m_TurnSpeed = 0; returnCam = secondsReturnCam; }
         else if (Input.GetAxis("Mouse X") > 0) { m_TurnSpeed = 0; returnCam = secondsReturnCam; }
         else if (returnCam >= 0) { returnCam--; }
         else if (returnCam <= 0) { m_TurnSpeed = 4; }

         // Axes Rotation - .Translate (X,Y,Z); 
         transform.Rotate(-RotateCamSpeed * Time.deltaTime * Input.GetAxis("Mouse Y"), RotateCamSpeed * Time.deltaTime * Input.GetAxis("Mouse X"), 0);
